...
export class FormulariosService {
    ...
    async createFormulario(
        ...
        const newForm = await FormulariosRepository.create(payload);

        // [FIX] Cria o vínculo na tabela de junção se houver id_tipo_demanda
        const anyInput = input as any;
        if (anyInput.id_tipo_demanda) {
            await DemandasTiposRepository.setFormularioLink(anyInput.id_tipo_demanda, newForm.id);
        }
    ...
    async updateFormulario(
        ...
        // [FIX] Atualiza o vínculo na tabela de junção se houver id_tipo_demanda
        const anyInput = input as any;
        if (anyInput.id_tipo_demanda) {
            await DemandasTiposRepository.setFormularioLink(anyInput.id_tipo_demanda, formId);
        }
