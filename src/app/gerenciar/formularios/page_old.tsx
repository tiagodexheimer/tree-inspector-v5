...
import { SortableItem } from "@/components/dnd_old/SortableItem";
...
const laudosSalvosExemplo = [
  ...
  {
    id: "laudo-003",
    nome: "Laudo de Supressão de Indivíduo",
...
export default function FormBuilderPage() {
  ...
  const updateField = (
    id: UniqueIdentifier,
    updatedProps: Partial<FormField>
  ) => {
    setFormFields((fields) =>
      fields.map((f) => (f.id === id ? { ...f, ...updatedProps } : f))
    );
  };
  ...
  const renderFormBuilder = () => (
    <DndContext onDragEnd={handleDragEnd} collisionDetection={closestCenter}>
      ...
      >
        ...
        <Paper
          sx={{ width: "50%", p: 2, display: "flex", flexDirection: "column" }}
        >
          <Typography variant="h6" gutterBottom>
            Estrutura do Laudo
          </Typography>
            ...
            {laudosSalvosExemplo.map((laudo) => (
              <TableRow key={laudo.id} hover>
                ...
                <TableCell align="right">
                  <IconButton title="Editar Laudo">
                    <EditIcon />
                  </IconButton>
                  <IconButton title="Apagar Laudo">
                    <DeleteIcon />
                  </IconButton>
                </TableCell>
              </TableRow>
