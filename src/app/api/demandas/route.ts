...
export async function GET(request: NextRequest) {
  ...
  if (userRole === "free_user") {
    console.log(
      "[API /demandas] Usuário 'free_user' detectado, limitando a 10 resultados (ignorando paginação)."
    );
    limit = 10;
  ...
  try {
    ...
    if (userRole !== "free_user") {
      const countResult = await pool.query(
        `SELECT COUNT(d.id) AS count ${baseQuery.replace(
          "ORDER BY d.created_at DESC",
          ""
        )}`
      );
      ...
      // [CORRIGIDO: Adicionado o alias 'd' no FROM demandas d]
      const countResult = await pool.query(
        `SELECT COUNT(d.id) AS count FROM demandas d`
      );
      totalCount = parseInt(countResult.rows[0].count, 10);
    }
...
export async function DELETE(request: NextRequest) {
  ...
  if (userRole !== "admin") {
    ...
    return NextResponse.json(
      {
        message:
          "Não autorizado: Apenas administradores podem deletar demandas.",
      },
      { status: 403 }
    );
  }
  ...
  try {
    ...
    const numericIds = idsToDelete.filter(
      (id) => typeof id === "number" && !isNaN(id)
    );
    if (numericIds.length !== idsToDelete.length) {
      return NextResponse.json(
        { message: "Array de IDs contém valores inválidos." },
        { status: 400 }
      );
    }
    ...
    const deleteRotasQuery =
      "DELETE FROM rotas_demandas WHERE demanda_id = ANY($1::int[])";
    await client.query(deleteRotasQuery, [numericIds]);

    // 2. Deletar da tabela principal 'demandas'
    const deleteDemandasQuery =
      "DELETE FROM demandas WHERE id = ANY($1::int[]) RETURNING id";
    ...
    if (deletedCount === 0) {
      console.warn(
        "[API /demandas] Nenhuma demanda encontrada para os IDs fornecidos:",
        numericIds
      );
      return NextResponse.json(
        { message: "Nenhuma demanda encontrada para os IDs fornecidos." },
        { status: 404 }
      );
    }

    console.log(
      `[API /demandas] ${deletedCount} demandas deletadas com sucesso.`
    );
    return NextResponse.json(
      {
        message: `${deletedCount} demandas deletadas com sucesso.`,
    ...
    console.error(
      "[API /demandas] Erro na transação de exclusão em massa:",
      error
    );
    const errorMessage =
      error instanceof Error ? error.message : "Erro desconhecido.";
    ...
    if (error instanceof Error && "code" in error && error.code === "23503") {
      console.warn(
        "[API /demandas] Tentativa de deletar demandas referenciadas em outras tabelas."
      );
      return NextResponse.json(
        {
          message:
            "Erro: Algumas demandas não puderam ser deletadas pois estão associadas a outros registros (ex: rotas não deletadas).",
          error: errorMessage,
        },
        { status: 409 }
      );
    ...
    return NextResponse.json(
      { message: "Erro interno ao deletar demandas.", error: errorMessage },
      { status: 500 }
    );
  } finally {
    client.release();
