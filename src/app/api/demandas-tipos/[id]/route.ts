...
async function checkAdminAccess() {
  ...
  if (!session || !session.user) {
    return { authorized: false, response: NextResponse.json({ message: "Não autenticado" }, { status: 401 }) };
  }
  if (session.user.role !== 'admin') {
    return { authorized: false, response: NextResponse.json({ message: "Não autorizado" }, { status: 403 }) };
  }
...
export async function PUT(request: NextRequest, context: ExpectedContext) {
  ...
  try {
    ...
    // Passa nome e id_formulario para o serviço
    const updatedTipo = await demandasTiposService.updateTipo(id, body.nome, body.id_formulario);
