...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json(
      { message: "Erro interno ao buscar rotas.", error: errorMessage },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!demandas || demandas.length === 0) {
      return NextResponse.json(
        { message: "A rota deve conter pelo menos uma demanda." },
        { status: 400 }
      );
    }
    ...
    demandas.forEach((demanda, index) => {
      ...
      demandaValues.push(
        `($1, $${placeholderIndex + 1}, $${placeholderIndex + 2})`
      );
    ...
    console.log(
      `[API /rotas] Rota ${rotaId} ("${nome}") criada com ${demandas.length} demandas.`
    );
    ...
    return NextResponse.json(
      { message: "Erro interno ao salvar a rota.", error: errorMessage },
      { status: 500 }
    );
  } finally {
    client.release();
